...
- [x] **EX-032** *(deployed 2026-08-22)* Archive-backed 90d/1y ranges on Analytics, reading across the retention boundary through one RPC so the caller never has to know where it sits. The range set is 24h/7d/90d/1y — 30d was removed on request, and because `LongRange` keys every table that describes a range, the type system found all five places it had to go — `src/lib/supabaseHistory.ts`, `src/components/analytics/AnalyticsPage.tsx`
...
- [x] **EX-038** The Devices fleet table becomes a four-band card per device below 720px — identity, status chips, labelled readings, action — instead of a 860px sideways-scrolling grid — scrolling right to read Power had taken the device's own name off screen. The nine `role="columnheader"` nodes are hidden visually but kept in the accessibility tree, so the row/column association EX-008 added survives — `src/components/devices/DevicesView.tsx`, `src/index.css`
