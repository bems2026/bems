...
test('GET /api/capabilities says whether the aircon can reach the cloud, and whether its local library is verified', async () => {
  ...
  try {
    ...
    // True since the on-site acceptance test (RM-120, 2026-09-22/23): the page says ON states go over
    // the LAN first, which is what the dispatcher now does.
    assert.equal(body.acu_local_ir_verified, true);
