...
| **Packaging** | Decided and built: `scripts/install.sh`, dry-run by default. **Both paths are now rehearsed on a machine with nothing installed** — `bash scripts/rehearse-install.sh [--apply]` runs them in a throwaway Debian container. The apply run completes 21 steps with no failures, and the artifacts are read back rather than taken on trust: Node 22, a clean build, Node-RED and the Tuya node, the loopback-only broker config byte-exact, `server/.env` at 600, and all four units rewritten for the new account and path. **`systemctl` is a recording stub there**, so nothing was enabled, nothing started, and no unit was validated by systemd — whether the services actually come up is still only knowable on a real machine. |
| ~~Day-one network setup~~ | **Covered** — `npm run preflight` checks credentials, the database, the vendor account (optional — a warning), the local radio segment, the bridge, the services, and what lives only on the host (persistent journal, recovery timers, a static address on every tuya node — `docs/outage-recovery.md`), and reports what it could *not* check rather than passing it. It does not perform the network join: it tells you whether one worked. |
