...
| `ibems-proxy` `ibems-scheduler` | the command path — `shared/commands.mjs`, `shared/acState.mjs`, `shared/buildLatest.mjs`, `shared/sitePolicy.mjs`, `server/dispatchLight.mjs`, `server/dispatchCloud.mjs`, `server/auditedDispatch.mjs`, and the vendor-cloud route the aircon may take from either — `server/cloudDispatchConfig.mjs`, `server/acRemote.mjs`, `server/tuyaCloud.mjs` |
| `ibems-ingest` | `server/ingest.mjs`, `server/reports.mjs`, `server/retention.mjs`, `server/ingestCycle.mjs`, `server/shapeRows.mjs`, `server/scrubTelemetry.mjs`, `server/readingCapabilities.mjs`, `server/healthRow.mjs`, `server/anomalyStats.mjs`, `server/fleetAlarm.mjs`, `server/supabaseRest.mjs`, `server/reportNotice.mjs`, and the report schedule the page reads too, `shared/reportSchedule.mjs` |
| `ibems-proxy` | `server/proxy.mjs`, `server/breakGlass.mjs`, `server/jwksCache.mjs`, `server/jwtVerify.mjs`, `server/livePolicy.mjs`, `server/macPresence.mjs`, `server/tuyaFleet.mjs`, `server/enrollRoute.mjs`, `server/enrollService.mjs`, `server/rebindRoute.mjs`, `server/rebindService.mjs`, `server/lanDiscovery.mjs`, `server/lanPresence.mjs`, `server/deviceSources.mjs`, `server/credentialStore.mjs`, `server/credentialImport.mjs`, `server/removeRoute.mjs`, `server/removeService.mjs`, `shared/enrollment.mjs`, `shared/tuyaNodeSettings.mjs` — and three from outside `server/` and `shared/`, `node-red-bridge/nodeRedAdmin.mjs`, `node-red-bridge/enrollPlan.mjs` and `node-red-bridge/rebindPlan.mjs` |
